...
const OrderCard = ({ order }: OrderCardProps) => {
  ...
  const cancelEnabled = !!process.env.NEXT_PUBLIC_CANCEL_PRODUCT || false
      ...
      <div className="divide-y-2 px-4">
        {order.items.map((product) => {
          return (
            <div
              key={product.id}
              className="flex max-sm:flex-col gap-4 py-4 overflow-hidden items-center"
            >
              ...
              <div className="flex flex-col gap-2">
                ...
                {returnEnabled && order.fulfillment_status === "shipped" && (
